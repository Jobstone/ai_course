...
# from common import settings,allowCROS
...
def find(s,step = 0,memory_name="default"):
    try:
        ...
        scores, indices = vectorstores[memory_name].index.search(np.array([embedding], dtype=np.float32), int(cunnrent_setting_count))
        docs = []
    ...
    try:
        ...
        try:
            vectorstores[memory_name] = FAISS.load_local(
                memory_path + memory_name, embeddings=embeddings)
            return vectorstores[memory_name]
        except Exception  as e:
...
def api_find():
    ...
    memory_name=data.get("memory_name")
    if step is None:
        step = int(settings_librarys_step)
